<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="rol" name="rol" type="text" class="mt-1 block w-full" :value="old('rol', Auth::user()->rol->nombre)"
                required autofocus autocomplete="name" readonly />
                <p class="mt-1 text-sm text-gray-600">
                    {{ __('Para cambiar el Rol comuniquese con un administrador con el motivo.') }}
                </p>
            <x-input-error class="mt-2" :messages="$errors->get('name')" />
        </div>
